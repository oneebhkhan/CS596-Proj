...
for (frame, rotation) in enumerate(rotations, 1):
	...
	xml_filepath = "Animation_Files/500_plants_rotation.xml"
	...
	with open("Animation_Files/500_plants_rotation.xml", "w") as field_xml:
		field_xml.write(reparsed_string)

	CAMERA = 0
	mitsuba_scene = load_file("Animation_Files/500_plants_rotation.xml")
	...
	img = film.bitmap(raw=True).convert(Bitmap.PixelFormat.RGB, Struct.Type.UInt8, \
		srgb_gamma=True)
	img.write("Animation_Files/field_python_{n:02d}.png".format(n=frame))